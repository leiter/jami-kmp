...
class LinkDetectionTest {
    ...
    fun plainText_noSpansAdded() {
        val result = LinkVisualTransformation(Color.Blue).filter(AnnotatedString("just plain text"))
    ...
    fun withUrl_blueUnderlineSpanAtUrlPosition() {
        ...
        val result = LinkVisualTransformation(Color.Blue).filter(AnnotatedString(text))
    ...
    fun sameInput_cachedResultReturnedByIdentity() {
        val transformation = LinkVisualTransformation(Color.Blue)

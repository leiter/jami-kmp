...
/**
 * Represents a contact-related event in a conversation.
 *
 * Events include: contact added, removed, blocked, invited, etc.
 *
...
class ContactEvent : Interaction {
    ...
    constructor(accountId: String, contact: Contact) : super() {
        ...
        event = Event.ADDED
        status = InteractionStatus.SUCCESS
        timestamp = contact.addedDate ?: currentTimeMillis()
        mIsRead = 1
    }

    constructor(accountId: String, contact: Contact, request: TrustRequest) : super() {
        account = accountId
    ...
    private fun getEventFromStatus(status: InteractionStatus): Event {
        return when (status) {
            InteractionStatus.SUCCESS -> Event.ADDED
            InteractionStatus.UNKNOWN -> Event.INCOMING_REQUEST
            else -> Event.UNKNOWN
        }
    }

...
- **Call transfer (attended)** — blind transfer UI is done; `attendedTransfer()` (transfer to a live second call) has no UI yet.
...
- **Ringtone applied to notification channel** — URI is persisted and shown in settings, but `AndroidNotificationService.createNotificationChannels()` does not yet read `CallSettings.ringtone` and apply it to the `jami_calls` channel at runtime (channel sound can only be set at creation on Android O+; requires deleting and recreating the channel on change).
...
- **AppSettingsScreen** — screenshot blocking enforcement is stored but not applied at the platform layer.
